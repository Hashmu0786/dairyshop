...
export default function AdminOrdersPage() {
  ...
  const filtered = filter === "all" ? enriched : enriched.filter((o) => o.status === filter);

  return (
    <>
      <PageHeader title="Orders" description="Manage all marketplace orders" />

      <div className="mb-6 flex flex-wrap gap-2">
        <button
          type="button"
          onClick={() => setFilter("all")}
          className={`rounded-full px-4 py-1.5 text-sm font-medium ${filter === "all" ? "bg-emerald-600 text-white" : "bg-white text-gray-600 border border-gray-200"}`}
        >
          All ({enriched.length})
        </button>
        {ORDER_STATUSES.map((s) => (
          <button
            ...
            type="button"
            onClick={() => setFilter(s)}
            className={`rounded-full px-4 py-1.5 text-sm font-medium capitalize ${filter === s ? "bg-emerald-600 text-white" : "bg-white text-gray-600 border border-gray-200"}`}
          >
            {s} ({enriched.filter((o) => o.status === s).length})
          </button>
        ))}
      </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-5 py-4 text-gray-700">{order.supplier?.name}</td>
